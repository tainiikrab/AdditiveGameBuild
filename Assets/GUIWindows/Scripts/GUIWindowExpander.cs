...
namespace Rellac.Windows
{
    ...
    public class GUIWindowExpander : GUIPointerObject
    {
        ...
        /// </summary>
        [Tooltip("")] [SerializeField] private RectTransform parentWindow;

        /// <summary>
        ///     Expander is locked and unusable
        /// </summary>
        [Tooltip("")] [SerializeField] private bool isLocked;

        /// <summary>
        ///     Allows you to double click this turningModel image to minimise/maximise
        /// </summary>
        [Tooltip("Allows you to double click this turningModel image to minimise/maximise")] [SerializeField]
        private bool doubleClick = true;

        /// <summary>
        ///     Fires when window starts restoring to a smaller size
        /// </summary>
        public UnityEvent onMinimised;

        /// <summary>
        ///     Fires when window starts expanding
        /// </summary>
        public UnityEvent onMaximised;
        ...
        private void Update()
        {
            if (doAction)
            {
                ...
                parentWindow.sizeDelta =
                    Vector2.MoveTowards(parentWindow.sizeDelta, targetSize, Time.deltaTime * 10000);
                parentWindow.anchoredPosition = Vector2.MoveTowards(parentWindow.anchoredPosition, targetPosition,
                    Time.deltaTime * 5000);
                // reached turningModel
                if (parentWindow.sizeDelta == targetSize && parentWindow.anchoredPosition == targetPosition)
                {
                    ...
                    if (isMaximised)
                    {
                        ...
                        if (onMaximised != null) onMaximised.Invoke();
                    }
                    else
                    {
                        // just invoke the event for a minimised window
                        if (onMinimised != null) onMinimised.Invoke();
                    }
                }
            }
        }
        ...
        public void MinimiseWindow()
        {
            ...
            if (initialPivot != Vector2.one * -1) parentWindow.SetPivot(initialPivot);
        ...
        public void TryDoubleClick()
        {
            ...
            else if (numClicks >= 2) Invoke("Swap", 0.1f); // wait in case we're interrupting a mover
        }
