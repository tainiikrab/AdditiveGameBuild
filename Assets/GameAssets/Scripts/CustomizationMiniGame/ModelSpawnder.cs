...
public class ModelSpawnder : MonoBehaviour
{
    ...
    private void Start()
    {
        if (OrderManager.orderData == null)
            modelPrefab = OrderManager.defaultOrderData.config.mesh;
        else
        ...
        foreach (var renderer in renderers)
        {
            ...
            for (var i = 0; i < mats.Length; i++) mats[i] = paintableMaterial;
        ...
        if (spawnedModel.GetComponent<PaintableObject>() == null) spawnedModel.AddComponent<PaintableObject>();

        if (spawnedModel.GetComponent<ObjectManipulator>() == null) spawnedModel.AddComponent<ObjectManipulator>();
    }
}